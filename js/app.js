...
function bind() {
  ...
  $("generalForm").addEventListener("submit", saveGeneral);
...
function showView(name) {
  ...
  const titles = { dashboard: "Resumen financiero", general: "Ingreso general", ingresos: "Ingresos", servicios: "Servicios", casa: "Pagos de casa", ahorros: "Ahorros", anual: "Balance anual" };
  $("pageTitle").textContent = titles[name];
  $("sidebar").classList.remove("open");
  if (name === "anual") loadAnnual();
  else if (name === "general") loadGeneral();
...
function refreshActiveView() {
  ...
  else if (id === "generalView") loadGeneral();
  else loadSummary();
}
...
function paintDistribution(t) {
  ...
  const rows = [["Ahorros", t.ahorros, "#6c55c7"], ["Servicios", t.servicios, "#d99416"], ["Pagos de casa", t.pagosCasa, "#2868d8"]];
...
function paintAnnualTable(rows) {
  $("annualTable").innerHTML = '<table class="data-table"><thead><tr><th>Mes</th><th>Ingresos</th><th>Ahorros</th><th>Servicios</th><th>Pagos de casa</th><th>Remanente</th></tr></thead><tbody>' + rows.map(r => '<tr><td><strong>' + r.nombre + '</strong></td><td>' + money(r.ingresos) + '</td><td>' + money(r.ahorros) + '</td><td>' + money(r.servicios) + '</td><td>' + money(r.pagosCasa) + '</td><td class="amount ' + (r.remanente >= 0 ? "positive" : "negative-text") + '">' + money(r.remanente) + '</td></tr>').join("") + '</tbody></table>';
}

function openModal(type) {
  const titles = { INGRESOS: "Registrar ingreso", SERVICIOS: "Registrar servicio", PAGOS_CASA: "Registrar pago de casa", AHORROS: "Registrar ahorro" };
  $("movementForm").reset();
...
function closeModal() {
  ...
  if (!$("generalModal").classList.contains("open")) $("overlay").classList.remove("open");
}
...
function labelType(t) {
  return ({ INGRESOS: "Ingreso", AHORROS: "Ahorro", SERVICIOS: "Servicio", PAGOS_CASA: "Pago de casa" })[t] || t;
}
